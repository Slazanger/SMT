...
public partial class EveManager
{
    ...
    /// <summary>
    /// Background thread cancellation flag 
    /// </summary>
    ...
    /// </summary>
    private readonly object _localCharactersLock = new();

    /// <summary>
    /// Observable collection for LocalCharacters that supports UI binding
    /// </summary>
    private readonly ObservableCollection<LocalCharacter> _localCharacters = new();
    ...
    /// </summary>
    private readonly object _esiRateLimitLock = new();

    /// <summary>
    /// Per-group ESI token bucket state (e.g. ErrorLimit). Updated from response headers.
    /// </summary>
    private Dictionary<string, EsiRateLimitBucketState> _esiRateLimitBuckets = new();
    ...
    public EveManager(string version)
    {
        ...
        var SaveDataRoot = EveAppConfig.StorageRoot;
        if (!Directory.Exists(SaveDataRoot)) Directory.CreateDirectory(SaveDataRoot);
        ...
        if (!Directory.Exists(SaveDataVersionFolder)) Directory.CreateDirectory(SaveDataVersionFolder);

        var characterSaveFolder = Path.Combine(SaveDataRootFolder, "Portraits");
        if (!Directory.Exists(characterSaveFolder)) Directory.CreateDirectory(characterSaveFolder);
    ...
    /// <summary>
    /// Gets or sets the EVE Standard API client (ESI wrapper) for making API calls to CCP's ESI endpoints.
    /// Initialized in the UI layer after loading saved settings and SSO credentials,
    /// since it depends on those for auth.
    ///
    /// Exposed here for use in background threads and other non-UI code that needs to make ESI calls
    /// without directly depending on UI classes.
    ///
    /// Note that the ESI client itself is thread-safe, but the rate-limit bucket state is managed
    /// separately in this class with its own locking to avoid coupling it to the ESI client's internal state.
    /// </summary>
    public EVEStandardAPI EveApiClient { get; set; }


    /// <summary>
    /// Gets or sets the Single Sign-On (SSO) v2 configuration.
    /// </summary>
    public SSOv2 Sso { get; set; }


    /// <summary>
    /// ESI Scopes that the user has authorized for this application (e.g. "esi-location.read_location.v1", "esi-clones.read_clones.v1", etc).
    /// </summary>
    ...
    /// </summary>
    public FixedQueue<IntelData> IntelDataList { get; set; }

    /// <summary>
    /// Gets or sets the Gamelog List
    /// </summary>
    public FixedQueue<GameLogData> GameLogList { get; set; }
    ...
    /// </summary>
    [XmlIgnore]
    public ObservableCollection<LocalCharacter> LocalCharacters => _localCharacters;
    ...
    public bool MoveLocalCharacterUp(LocalCharacter character)
    {
        if (UIThreadInvoker != null)
        {
            ...
            UIThreadInvoker(() =>
            {
                lock (_localCharactersLock)
                {
                    var index = _localCharacters.IndexOf(character);
                    if (index > 0)
                    {
    ...
    /// <summary>
    /// Use ESI to get character positions for the local characters instead of parsing them from the local log files.
    /// </summary>
    public bool UseESIForCharacterPositions { get; set; }


    /// <summary>
    /// List of active Metaliminal Storms 
    /// </summary>
    public List<Storm> MetaliminalStorms { get; set; }


    /// <summary>
    /// List of static points of interests
    /// </summary>
