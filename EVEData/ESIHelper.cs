...
public class ESIHelpers
{
    ...
    public static void TryUpdateEsiRateLimitFromResponse<T>(ESIModelDTO<T> response)
    {
        ...
        try
        {
            ...
            var remain = GetIntProperty(type, response, "ErrorLimitRemain")
                         ?? GetIntProperty(type, response, "RequestLimitRemain")
                         ?? GetIntProperty(type, response, "RateLimitRemaining");
            var resetSeconds = GetIntProperty(type, response, "ErrorLimitReset")
            ...
            {
                var group = GetStringProperty(type, response, "RateLimitGroup")
    ...
    private static int? GetIntProperty(Type type, object obj, string propertyName)
    {
        ...
        if (pt != typeof(int) && pt != typeof(int?) && !(pt.IsGenericType &&
                                                         pt.GetGenericTypeDefinition() == typeof(Nullable<>) &&
                                                         pt.GetGenericArguments()[0] == typeof(int)))
            return null;
