...
public class ZKillR2Z2
{
    ...
    {
        if (!backgroundWorker.IsBusy && !PauseUpdate && DateTime.Now >= nextPollTime) backgroundWorker.RunWorkerAsync();
    }
    ...
    private void zkb_DoWork(object sender, DoWorkEventArgs e)
    {
        try
        {
            ...
            {
                var seqUrl = "https://r2z2.zkillboard.com/ephemeral/sequence.json";
                var seqResponse = hc.GetAsync(seqUrl).Result;
                if (seqResponse.IsSuccessStatusCode)
                {
                    var seqContent = seqResponse.Content.ReadAsStringAsync().Result;
                    var seqData = SequenceData.FromJson(seqContent);
                    if (seqData != null) currentSequence = seqData.Sequence;
                }
            ...
            var r2z2Url = $"https://r2z2.zkillboard.com/ephemeral/{currentSequence}.json";
            ...
            else if (response.IsSuccessStatusCode)
            {
                ...
                var r2z2Data = R2Z2Data.FromJson(strContent);

                if (r2z2Data != null && r2z2Data.Esi != null && r2z2Data.Esi.Victim != null)
                {
                    var zs = new ZKBDataSimple();
                    ...
                    var shipID = r2z2Data.Esi.Victim.ShipTypeId.ToString();
                    ...
                    if (KillsAddedEvent != null) KillsAddedEvent();
                }
    ...
    {
        var updatedKillList = false;

        var AllianceIDs = new List<int>();

        for (var i = KillStream.Count - 1; i >= 0; i--)
        {
            if (KillStream[i].VictimAllianceName == string.Empty)
            {
                if (!EveManager.Instance.AllianceIDToTicker.ContainsKey(KillStream[i].VictimAllianceID) &&
                    !AllianceIDs.Contains(KillStream[i].VictimAllianceID) && KillStream[i].VictimAllianceID != 0)
                    AllianceIDs.Add(KillStream[i].VictimAllianceID);
                else
                    KillStream[i].VictimAllianceName =
                        EveManager.Instance.GetAllianceName(KillStream[i].VictimAllianceID);
            }
    ...
    public class ZKBDataSimple : INotifyPropertyChanged
    {
        ...
        {
            var allianceTicker = EveManager.Instance.GetAllianceTicker(VictimAllianceID);
            if (allianceTicker == string.Empty) allianceTicker = VictimAllianceID.ToString();
