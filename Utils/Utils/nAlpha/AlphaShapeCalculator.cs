namespace Utils.nAlpha
{
    ...
    {
        private readonly List<Tuple<int, int>> resultingEdges = new();
        private readonly List<Point> resultingVertices = new();
        ...
        {
            foreach (var point in points) ProcessPoint(point);
        }

        private VertexCounter CountVertices()
        {
            var counter = new VertexCounter();
        ...
        private int GetNearestPendingVertex(int[] vertices, int vertexIndex)
        {
            ...
            var vertexIndicesWithDistance =
                vertices.Where(v => v != vertexIndex).Select(v => new
                    { Index = v, Distance = resultingVertices[v].DistanceTo(vertexPoint) });
        ...
        private void ProcessPoint(Point point)
        {
            ...
            {
                var alphaDiskCenters = CalculateAlphaDiskCenters(point, otherPoint);
        ...
        {
            int index;
            if (!resultingVertices.Contains(p)) resultingVertices.Add(p);
        ...
        {
            var distanceBetweenPoints = p1.DistanceTo(p2);
            var distanceFromConnectionLine =
                Math.Sqrt(Radius * Radius - distanceBetweenPoints * distanceBetweenPoints / 4);

            var centerOfConnectionLine = p1.CenterTo(p2);
            var vector = p1.VectorTo(p2);

            return GetAlphaDiskCenters(vector, centerOfConnectionLine, distanceFromConnectionLine);
        }

        private static Tuple<Point, Point> GetAlphaDiskCenters(Point vector, Point center,
            double distanceFromConnectionLine)
        {
            var normalVector = new Point(vector.Y, -vector.X);
            return
