...
namespace Utils.csDelaunay.Delaunay
{
    public class Halfedge
    {
        ...
        {
            if (pool.Count > 0) return pool.Dequeue().Init(edge, lr);
        ...
        public bool IsLeftOf(Vector2f p)
        {
            ...
            if (rightOfSite && leftRight == LR.LEFT) return true;
            if (!rightOfSite && leftRight == LR.RIGHT) return false;
